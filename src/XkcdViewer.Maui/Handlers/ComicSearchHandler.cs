        ...
        switch (Shell.Current.CurrentPage)
        {
            case MainPage mp:
                if (mp.BindingContext is MainPageViewModel mainVm)
                {
                    ...
                    {
                        ItemsSource = mainVm.Comics
                            .Where(c => c.Title.ToLower().Contains(searchTerm.ToLower()) || c.Num.ToString().Contains(searchTerm.ToLower()))
                            .ToList();
                    }
                    else
                    {
            ...
            case FavoritesPage fp:
                if (fp.BindingContext is FavoritesPageViewModel favVm)
                {
                    if (item is string searchTerm)
                    {
                        ItemsSource = favVm.FavoriteComics
                            .Where(c => c.Title.ToLower().Contains(searchTerm.ToLower()) || c.Num.ToString().Contains(searchTerm.ToLower()))
                            .ToList();
                    }
                    else
